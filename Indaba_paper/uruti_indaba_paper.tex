...
\usepackage{multirow}
\usepackage{placeins}  % \FloatBarrier — stops floats crossing subsection boundaries
\usepackage{xcolor}
\usepackage{tikz}
\usepackage{pgfplots}
...
% Args: #1=title #2=NR-NR #3=NR-MN #4=NR-IR
%       #5=MN-NR #6=MN-MN #7=MN-IR #8=IR-NR #9=IR-MN
% IR-IR = 861-#8-#9
\newcommand{\onesinglecm}[9]{%
\begin{tikzpicture}[every node/.style={font=\tiny}]
\def\cs{1.0}%  % cell size — smaller so labels have room outside
% IR-IR count
\pgfmathtruncatemacro{\irir}{861-#8-#9}%
% Row totals for per-row normalisation
\pgfmathsetmacro{\Rnr}{max(1,#2+#3+#4)}%
\pgfmathsetmacro{\Rmn}{max(1,#5+#6+#7)}%
\pgfmathsetmacro{\Rir}{max(1,#8+#9+\irir)}%
% Per-cell intensity 0-100
...
% Text-colour flags: 1=white (dark cell) 0=black (light cell)
...
% ─── Cells: row 0=not_ready (bottom), 1=mentorship_needed, 2=investment_ready (top)
\node[draw,minimum size=\cs cm,inner sep=0,fill=blue!\Iaa!white] at (0,0)        {\ifnum\Taa=1\textcolor{white}{#2}\else#2\fi};
\node[draw,minimum size=\cs cm,inner sep=0,fill=blue!\Iab!white] at (\cs,0)      {\ifnum\Tab=1\textcolor{white}{#3}\else#3\fi};
\node[draw,minimum size=\cs cm,inner sep=0,fill=blue!\Iac!white] at (2*\cs,0)    {\ifnum\Tac=1\textcolor{white}{#4}\else#4\fi};
\node[draw,minimum size=\cs cm,inner sep=0,fill=blue!\Iba!white] at (0,\cs)      {\ifnum\Tba=1\textcolor{white}{#5}\else#5\fi};
\node[draw,minimum size=\cs cm,inner sep=0,fill=blue!\Ibb!white] at (\cs,\cs)    {\ifnum\Tbb=1\textcolor{white}{#6}\else#6\fi};
\node[draw,minimum size=\cs cm,inner sep=0,fill=blue!\Ibc!white] at (2*\cs,\cs)  {\ifnum\Tbc=1\textcolor{white}{#7}\else#7\fi};
\node[draw,minimum size=\cs cm,inner sep=0,fill=blue!\Ica!white] at (0,2*\cs)    {\ifnum\Tca=1\textcolor{white}{#8}\else#8\fi};
\node[draw,minimum size=\cs cm,inner sep=0,fill=blue!\Icb!white] at (\cs,2*\cs)  {\ifnum\Tcb=1\textcolor{white}{#9}\else#9\fi};
\node[draw,minimum size=\cs cm,inner sep=0,fill=blue!\Icc!white] at (2*\cs,2*\cs){\ifnum\Tcc=1\textcolor{white}{\irir}\else\irir\fi};
% ─── X-axis labels (Predicted): anchor=north east, rotate=45
%     Placed 0.45cm below cell bottom (cell bottom at y=-cs/2=-0.5)
...
% ─── Y-axis labels (True): anchor=east, 0.25cm left of cell edge (cell left at x=-0.5)
...
its startup ecosystem has attracted growing venture capital inflows and generated
substantial employment opportunities~\citep{partechafrica2024}, yet the pathways from
...
this gap — remain severely under-resourced relative to demand, leaving the overwhelming majority
of promising founders without mentorship, pitch preparation, or objective
evaluation~\citep{gem2022africa}.
...
harmful in high-stakes advisory contexts~\citep{ji2023hallucination}.
...
capital support. \citet{ouyang2022instructgpt} showed
that RLHF-trained models produce measurably better goal-aligned outputs
than non-fine-tuned baselines across structured feedback tasks. Similarly,
\citet{lewis2020rag} demonstrated that retrieval-augmented transformer-based
systems can substantially reduce knowledge gaps in advisory interactions.
However, neither approach was grounded in an African regulatory
...
\citet{grinsztajn2022tabular} showed that gradient-boosting ensembles
...
non-linear feature interactions dominate --- precisely the pattern we observe
...
\citet{dettmers2023qlora} demonstrated that QLoRA-tuned
open-source models can match or exceed much larger generic models at a fraction
of the compute cost, enabling practical fine-tuning on single-GPU budgets.
The Low-Rank Adaptation (LoRA)~\citep{hu2022lora} technique reduces the trainable parameter
footprint by 99\%, preserving the base model's general reasoning capabilities.
...
\begin{axis}[
    ybar, bar width=12pt,
    width=\columnwidth, height=7.0cm,
    ylabel={F1-Weighted Score}, ylabel style={font=\small},
    xlabel={Model Names}, xlabel style={font=\small},
    ...
    tick label style={font=\scriptsize},
    title={Model Performance Comparison (F1-Weighted Score)},
    title style={font=\small\bfseries},
    legend style={font=\tiny, at={(0.98,0.98)}, anchor=north east},
]
\addplot[fill=red!80, fill opacity=0.9]
  coordinates {(1,0.8382)};
\addlegendentry{Top Performer}
\addplot[fill=blue!70!black, fill opacity=0.9]
  coordinates {(2,0.7625)(3,0.7999)(4,0.7532)(5,0.7827)(6,0.7800)(7,0.7801)(8,0.7993)};
\addlegendentry{Other Models}
\end{axis}
\end{tikzpicture}
\caption{Model Performance Comparison (F1-Weighted Score) for all 8 models
         on the held-out test set (11{,}054 records). XGBoost (0.8382) is
         highlighted as top performer (red); all other models shown in blue.
         Models cluster tightly between 0.75--0.84, revealing that the dominant
         challenge is minority-class recall rather than overall accuracy.}
...
Fig.~\ref{fig:investor_f1}, Figs.~\ref{fig:cm_lr}--\ref{fig:cm_cnn_c}).
