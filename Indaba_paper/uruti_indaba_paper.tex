...
Figure~\ref{fig:eda} presents four-panel exploratory analysis of the harmonised
55{,}267-record corpus (Figure~\ref{fig:eda}).
The class distribution (top-left) confirms severe imbalance:
...
The data-source breakdown (top-right) reveals that \textit{investments\_VC.csv}
dominates at 98.3\% of records.
The funding distribution by class (bottom-left) shows right-skewed spread,
with investment-ready firms exhibiting higher median funding.
The Pearson correlation heatmap (bottom-right) reveals
...
\begin{figure*}[t]
\centering
%% Row 1: class distribution (left) + source breakdown (right)
\begin{tikzpicture}
\begin{axis}[
    name=ax1,
    ybar, bar width=18pt,
    ...
    title={Investor Readiness Class Distribution},
...
\begin{axis}[
    ...
    grid=major, grid style={dotted, gray!40},
    tick label style={font=\scriptsize},
    title={Readiness Class by Data Source},
    title style={font=\small\bfseries},
...
%% Row 2: funding boxplot (left, approximated as scatter) + correlation heatmap (right)
\begin{tikzpicture}
\begin{axis}[
    name=ax3,
...
%% IQR bars (error bars approximating boxes)
...
\end{tikzpicture}%
\hfill
%% Correlation heatmap via matrix plot
\begin{tikzpicture}[
    every node/.style={font=\tiny},
    hcell/.style={minimum size=0.92cm, inner sep=0pt, font=\tiny, text=white}
]
\def\labels{{"funding","rounds","founded","profit","readiness"}}
% Row 0: funding_total_usd
...
\end{tikzpicture}
\caption{Investor Readiness Corpus EDA (55{,}267 records).
         \textbf{Top-left:} Class distribution --- 86.88\% is
         \texttt{mentorship\_needed}, exposing severe imbalance requiring SMOTE.
         \textbf{Top-right:} Contribution by data source; \textit{investments\_VC.csv}
         provides 98.3\% of records.
         \textbf{Bottom-left:} Funding distribution by readiness class.
         \textbf{Bottom-right:} Pearson correlation heatmap; profit--readiness
         ($r$=0.88) and funding--profit ($r$=0.87) are the dominant predictive signals.}
\label{fig:eda}
\end{figure*}
...
\begin{table}[t]
\centering
\caption{Full model comparison on the held-out test set (11{,}054 records).
         F1$_w$ = weighted F1-score; F1$_m$ = macro F1-score; Bal.\ Acc =
         balanced accuracy. Values sourced from classification report outputs.}
\label{tab:investor_results}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{Acc.} & \textbf{F1$_w$} & \textbf{F1$_m$} & \textbf{Deploy}\\
\midrule
XGBoost $\star$        & 88.10\% & \textbf{83.82\%} & 42.70\% & \checkmark \\
Logistic Regression    & 87.90\% & 83.47\% & 40.96\% & --- \\
SVC                    & 87.93\% & ---     & ---     & --- \\
CNN\_C (SGD, lr=1e-2)  & 87.91\% & 83.22\% & ---     & --- \\
CNN\_B (RMSprop)       & 87.86\% & 83.07\% & ---     & --- \\
CNN\_A (Adam, lr=1e-3) & 86.95\% & 80.95\% & ---     & --- \\
Random Forest          & 86.04\% & ---     & ---     & --- \\
ExtraTrees             & 83.22\% & ---     & ---     & --- \\
\bottomrule
\end{tabular}%
...
Figure~\ref{fig:cm_classical} shows confusion matrices for the five classical
classifiers on the held-out test set (11{,}054 records; class counts:
\texttt{not\_ready} 589, \texttt{mentorship\_needed} 9{,}604,
\texttt{investment\_ready} 861).
XGBoost (bottom-right) achieves the most balanced diagonal, correctly
...
\begin{figure*}[t]
\centering
% 5 matrices in two rows: 3 top, 2 bottom centred (macro defined in preamble)
...
\hspace{0.18\textwidth}%
\caption{Confusion matrices for the five classical models on the 11{,}054-record
         held-out set.
         Classes shown on axes: \texttt{not\_ready} (NR, 589 instances),
         \texttt{mentorship\_needed} (MN, 9{,}604), \texttt{investment\_ready}
         (IR, 861).
         XGBoost (bottom-right) achieves the highest minority recall.
         All models are dominated by the 86.9\% majority class prior.}
...
\begin{figure*}[t]
\centering
% Three distribution histograms side by side
\begin{tikzpicture}
\begin{axis}[
    name=dist1,
    width=0.31\textwidth, height=3.8cm,
    ...
    ymin=0, ymax=320,
    tick label style={font=\scriptsize},
    title={Target Sentiment Score},
    title style={font=\small\bfseries},
    grid=major, grid style={dotted,gray!40},
]
% Right-skewed distribution centred ~0.25
...
\begin{axis}[
    ...
    ymin=0, ymax=420,
    tick label style={font=\scriptsize},
    title={Eye-Contact Score (Facet42)},
    title style={font=\small\bfseries},
    grid=major, grid style={dotted,gray!40},
]
% Low-mean, right-heavy in low values
...
\begin{axis}[
    ...
    ymin=0, ymax=380,
    tick label style={font=\scriptsize},
    title={Vocal Energy / RMS (COVAREP)},
    title style={font=\small\bfseries},
    grid=major, grid style={dotted,gray!40},
]
% Sparse -- concentrated near zero
...
\end{axis}
\end{tikzpicture}
\caption{Marginal distributions of three CMU-MOSEI pitch features (1{,}000
         aligned segments).
         \textbf{Left:} Target sentiment score (right-skewed; mean $\approx$\,0.25).
         \textbf{Centre:} Eye-contact score (Facet42; low-mean, concentrated near zero).
         \textbf{Right:} Vocal energy / RMS (COVAREP; sparse in this subset).}
...
\begin{equation}
\boxed{
r_t = 4.0 \cdot \underbrace{(\text{score}_t - \text{score}_{t-1})}_{\text{delta reward}}
+ \underbrace{2.0 \cdot \mathbf{1}[\text{score}_t > 85]}_{\text{maintenance bonus}}
- \underbrace{\lambda_p \cdot \max(0,\, \Delta t - \bar{t}_s)}_{\text{pacing penalty}}
}
\label{eq:reward}
\end{equation}
...
\textbf{83.82\%} on 11{,}054 held-out records, outperforming all five CNN
variants (Table~\ref{tab:investor_results}).
The corrected accuracy from notebook output is 88.10\%, reconciling earlier
reported figures.
Figure~\ref{fig:cm_cnn} compared CNN confusion matrices confirm that all three
CNN variants collapse minority-class predictions into \texttt{mentorship\_needed}:
CNN\_A classifies only 6/861 investment-ready startups correctly (recall 0.7\%),
while XGBoost achieves 117/861 (recall 13.6\%).
...
\begin{figure*}[t]
\centering
% --- CNN confusion matrices: 3 side-by-side ---
% Reuse \onesinglecm defined above
...
\caption{Confusion matrices for three 1D-CNN variants on the 11{,}054-record
         held-out set.
         \textbf{Left} CNN\_A (Adam, lr=1e-3, 20~ep): only 6/861 investment-ready
         correctly classified --- total majority-class collapse.
         \textbf{Centre} CNN\_B (RMSprop, lr=5e-4, 30~ep): marginally improved
         minority recall ($\sim$108/861 investment-ready).
         \textbf{Right} CNN\_C (SGD, lr=1e-2, 40~ep): best CNN, 103/861
         investment-ready and 12/589 not-ready correct.
         All three CNNs are dominated by the 86.9\% majority class despite SMOTE;
         XGBoost (Fig.~\ref{fig:cm_classical}) provides vastly superior minority
         recall.}
\label{fig:cm_cnn}
\end{figure*}
