...
% X-axis tick labels
\foreach \x/\lbl in {0/not\_ready,1/mentorship\\needed,2/investment\_ready}
  \node[rotate=30,anchor=north west,font=\tiny,text width=1.8cm,align=left] at (\x*\cs-0.35,-0.62) {\lbl};
% Y-axis tick labels
\foreach \y/\lbl in {0/not\_ready,1/mentorship\\needed,2/investment\_ready}
  \node[anchor=east,font=\tiny,text width=1.6cm,align=right] at (-0.58,\y*\cs) {\lbl};
% Axis names
\node[font=\tiny,rotate=90,anchor=south] at (-1.6,\cs) {True label};
\node[font=\tiny,anchor=north]            at (\cs,-1.7) {Predicted label};
% Title
...
\begin{axis}[
    ...
    symbolic x coords={
      XGBoost,
      {Logistic\\Regression},
      {CNN\_C\\SGD lr1e-2\\ep40},
      SVC,
      {Random\\Forest},
      {CNN\_B\\RMSprop\\lr5e-4\\ep30},
      {Decision\\Forest\\(ExtraTrees)},
      {CNN\_A\\Adam\\lr1e-3\\ep20}},
    xtick=data,
...
% XGBoost — top performer in red
\addplot[fill=red!75, fill opacity=0.9] coordinates {(XGBoost, 0.8382)};
\addlegendentry{Top Performer}
% All other models in navy blue
\addplot[fill=blue!70!black, fill opacity=0.85] coordinates {
  ({Logistic\\Regression},         0.7625)
  ({CNN\_C\\SGD lr1e-2\\ep40},     0.7999)
  (SVC,                             0.7532)
  ({Random\\Forest},                0.7827)
  ({CNN\_B\\RMSprop\\lr5e-4\\ep30}, 0.7800)
  ({Decision\\Forest\\(ExtraTrees)},0.7801)
  ({CNN\_A\\Adam\\lr1e-3\\ep20},    0.7993)
};
