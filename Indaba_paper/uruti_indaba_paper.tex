...
% ─── Cells: row 0 = true not_ready (bottom), 1 = mentorship_needed, 2 = investment_ready (top)
...
% ─── X-axis labels (Predicted) ─ rotated 40°
\node[rotate=40,anchor=north west,font=\tiny] at (0*\cs-0.25,-0.65) {not\_ready};
\node[rotate=40,anchor=north west,font=\tiny] at (1*\cs-0.25,-0.65) {mentorship\_needed};
\node[rotate=40,anchor=north west,font=\tiny] at (2*\cs-0.25,-0.65) {investment\_ready};
% ─── Y-axis labels (True)
\node[anchor=east,font=\tiny] at (-0.65,0)       {not\_ready};
\node[anchor=east,font=\tiny] at (-0.65,\cs)     {mentorship\_needed};
\node[anchor=east,font=\tiny] at (-0.65,2*\cs)   {investment\_ready};
% ─── Axis name labels
\node[font=\tiny,rotate=90,anchor=south] at (-2.5,\cs)  {True label};
\node[font=\tiny,anchor=north]           at (\cs,-2.1)   {Predicted label};
% ─── Figure title
\node[font=\scriptsize\bfseries,anchor=south,text width=4.2cm,align=center]
  at (\cs,3*\cs+0.2) {#1};
\end{tikzpicture}%
}
