...
export function SignupPage({ onNavigate, onSignupComplete }: SignupPageProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    // Step 3a: Founder Profile (Optional)
    bio: '',
    linkedin: '',
    twitter: '',
    website: '',
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await signup(formData.email, formData.fullName, formData.password, userType);

      // Note: Profile details will be saved after first login
      // Store profile data in localStorage temporarily if provided
      if (userType === 'founder') {
        const hasProfileData = formData.bio || formData.linkedin || formData.twitter || formData.website || formData.skills;
        if (hasProfileData) {
          const profileData: any = {};
          if (formData.bio) profileData.bio = formData.bio;
          if (formData.linkedin) profileData.linkedin_url = formData.linkedin;
          if (formData.twitter) profileData.twitter_url = formData.twitter;
          if (formData.website) profileData.website_url = formData.website;
          if (formData.skills) profileData.skills = formData.skills.split(',').map(s => s.trim());
          
          // Store temporarily to update after login
        ...
        if (formData.organizationName) profileData.organization = formData.organizationName;
        if (formData.investorType) profileData.investor_type = formData.investorType;
        if (formData.investmentRange) profileData.investment_range = formData.investmentRange;
        if (formData.sectors) profileData.preferred_sectors = formData.sectors.split(',').map(s => s.trim());
        if (formData.investmentStage) profileData.investment_stage = formData.investmentStage;
        if (formData.portfolioSize) profileData.portfolio_size = formData.portfolioSize;
        if (formData.investmentCriteria) profileData.investment_thesis = formData.investmentCriteria;
  ...
  const handleSkipProfile = async () => {
    // Skip to account creation without profile details
              ...
              {currentStep === 3 && userType === 'founder' && (
                <div className="space-y-6">
                  <div className="text-center mb-8">
                    ...
                    <p className="text-muted-foreground" style={{ fontFamily: 'var(--font-body)' }}>
                      Help us personalize your experience (Optional - you can skip this step)
                    </p>
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="bio" className="flex items-center space-x-2">
                      <FileText className="h-4 w-4 text-[#76B947]" />
                      <span>Bio</span>
                    </Label>
                    <Textarea
                      id="bio"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="skills" className="flex items-center space-x-2">
                      <Briefcase className="h-4 w-4 text-[#76B947]" />
                  ...
                  <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 p-4">
                    <p className="text-sm text-blue-800 dark:text-blue-300" style={{ fontFamily: 'var(--font-body)' }}>
                      <strong>Note:</strong> You can add your startup details later in the Startup Lab module. Focus on building your profile first!
                    </p>
                  </div>
                </div>
              )}
                    ...
                    <>
                      {currentStep === totalSteps && (
                        <Button
                          ...
                          disabled={isSubmitting}
                          className="flex-1 border-[#76B947] text-[#76B947] hover:bg-[#76B947]/10"
                        >
                          Skip for Now
                        </Button>
                      )}
